...
const ClinicVisitGrid = () => {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      adjustedResponse = adjustedResponse.sort(
  ...
  const filteredDocumentData = documents.filter(
    (document) =>
      (document._id ? document._id.toLowerCase() : "").includes(searchValue) ||
      (document.patient_id ? document.patient_id.toLowerCase() : "").includes(
        searchValue
      ) ||
      (document.name ? document.name.toLowerCase() : "").includes(
        searchValue
      ) ||
      (document.patient_age ? document.patient_age : "").includes(
        searchValue
      ) ||
      ...
      (document.pcp ? document.pcp.toLowerCase() : "").includes(searchValue) ||
      (document.medicine ? document.medicine.toLowerCase() : "").includes(
        searchValue
      ) ||
      (document.quantity ? document.quantity : "").includes(searchValue) ||
      (document.dosage ? document.dosage.toLowerCase() : "").includes(
        searchValue
      ) ||
      (document.frequency ? document.frequency.toLowerCase() : "").includes(
        searchValue
      ) ||
      (document.duration ? document.duration.toLowerCase() : "").includes(
        searchValue
      ) ||
      ...
      (document.reason ? document.reason.toLowerCase() : "").includes(
        searchValue
      ) ||
