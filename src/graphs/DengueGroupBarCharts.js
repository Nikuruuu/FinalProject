...
const BarChart = () => {
      ...
      try {
        ...
        if (!selectedSchoolYear && sortedSchoolYears.length > 0) {
          setSelectedSchoolYear(
            `${sortedSchoolYears[0].syStartYear} - ${sortedSchoolYears[0].syEndYear}`
          );
          specifiedDateData(
            `${sortedSchoolYears[0].syStartYear} - ${sortedSchoolYears[0].syEndYear}`
          );
      ...
      ...new Set(
        originalData
          .map((item) => {
            ...
            return null; // or a default value like "Unknown"
          })
          .filter((grade) => grade !== null) // Filter out nulls
      ),
    ];
    return uniqueTypes;
  };
  ...
  const aggregateDataByReason = useCallback((rawData) => {
    ...
    rawData.forEach((item) => {
      const distinctChartData =
        item.classEnrollment.classProfile.section.toUpperCase();
  ...
  const specifiedDateData = (selectedYear, selectedMonth, selectedType) => {
    if (selectedYear) {
      ...
      const startMonth =
        schoolYears.find((year) => year.syStartYear === startYear)
          ?.syStartMonth || 1;
      const endMonth =
        schoolYears.find((year) => year.syEndYear === endYear)?.syEndMonth ||
        12;
      ...
      const filteredData = originalData.filter((item) => {
        ...
        if (selectedMonth === "All") {
          ...
          if (
            (basedDateYr === startYear &&
              basedDateMth >= startMonthIndex &&
              basedDateMth === selectedMonthIndex) ||
            (basedDateYr === endYear &&
              basedDateMth <= endMonthIndex &&
              basedDateMth === selectedMonthIndex)
          ) {
            return true;
          }
      ...
      const typeFilter =
        selectedType === "All"
          ? filteredData
          : filteredData.filter(
              (item) => item.classEnrollment.classProfile.grade === selectedType
            );
  ...
  const summary = () => {
    ...
    const selectedMonthText =
      selectedMonth === "All" ? "all months" : `the month of ${selectedMonth}`;
    const selectedTypeText =
      selectedType === "All" ? "all grades" : `the ${selectedType}`;
    ...
    if (highestTypes.length === 1) {
      ...
      const highestTypeLabels = highestTypes
        .map((item) => item.label)
        .join(", ");
          ...
          <Paper elevation={3}>
            <Box p={3}>
              ...
              <Typography variant="body1" paragraph>
                It is Bar Chart that provides a focused identifying Dengue
                infected student/s on each section, allowing you to filter data
                by school year, month, and grade.
              </Typography>
                  ...
                  <FormControl variant="outlined" fullWidth>
                    ...
                    >
                      {schoolYears.map((year) => (
                        <MenuItem
                          key={year._id}
                          value={`${year.syStartYear} - ${year.syEndYear}`}
                        >
