...
const LineChart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const selectedYearStart =
          sy.find((data) => data._id === selectedSy)?.startYr ||
          new Date().getFullYear();
        const selectedYearEnd =
          sy.find((data) => data._id === selectedSy)?.endYr ||
          new Date().getFullYear();

        // Assuming the default start month is January (1) and end month is December (12).
        const selectedStartMonth =
          sy.find((data) => data._id === selectedSy)?.startMth || 1;
        const selectedEndMonth =
          sy.find((data) => data._id === selectedSy)?.endMth || 12;
  ...
  const summary = () => {
    ...
    function generateMessage(
      ...
      const dateInfo = isAllMonth
        ? monthData.months
        : monthData.days.map((day) => day.slice(-2));
      ...
      const dateList =
        dateInfo.length === 1
          ...
          : `are ${dateInfo.join(", ")}`;
      return isAllMonth
        ? `In the School Year ${schoolYearText}, the ${dateLabel} registering the highest number of record(s) ${dateList}, reflecting ${monthData.count} Dengue infection/s. This surge predominantly pertains to the ${typeLabel}, signifying a prominent trend in patient interactions during this period.`
        : `In the School Year ${schoolYearText}, the ${dateLabel} registering the highest number of record(s) ${dateList}, reflecting ${monthData.count} Dengue infection/s. This surge predominantly pertains to the ${typeLabel}, signifying a prominent trend in patient interactions during this period.`;
    }
          ...
          <Paper elevation={3}>
            <Box p={3}>
              <Typography variant="h4" gutterBottom>
                Dengue Monitoring By Grade Analysis
              </Typography>
              <Typography variant="body1" paragraph>
                It is Multiline Chart that provides a focused identifying of
                Dengue infected student trends on each grade, allowing you to
                filter data by school year, month, and grade.
              </Typography>
                  ...
                  <ResponsiveLine
                    ...
                    xScale={{
                      ...
                      format: (value) => {
                        ...
                        const abbreviatedMonth = monthNames[monthIndex]?.slice(
                          0,
                          3
                        );
                        if (xAxisFormat === "Month") {
                          return `${year}-${
                            abbreviatedMonth || monthNames[monthIndex]
                          }`;
                        } else {
                          return `${year}-${
                            abbreviatedMonth || monthNames[monthIndex]
                    ...
                    axisBottom={{
                      ...
                      format: (value) => {
                        if (xAxisFormat === "Month") {
                          const [year, month] = value.split("-");
                          const monthIndex = parseInt(month) - 1;
                          const abbreviatedMonth = monthNames[
                            monthIndex
                          ]?.slice(0, 3);
                          return `${
                            abbreviatedMonth || monthNames[monthIndex]
