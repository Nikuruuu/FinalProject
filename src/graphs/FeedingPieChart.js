...
const PieChart = () => {
  ...
  const extractTypes = () => {
    const uniqueTypes = [
      ...new Set(originalData.map((item) => item.measurementType)),
    ];
    return uniqueTypes;
  };
    ...
    if (selectedYear) {
      ...
      const filteredData = originalData.filter((item) => {
        ...
        if (selectedMonth === "All") {
          ...
          if (
            (basedYear === startYear &&
              basedMonth >= startMonthIndex &&
              basedMonth === selectedMonthIndex) ||
            (basedYear === endYear &&
              basedMonth <= endMonthIndex &&
              basedMonth === selectedMonthIndex)
          ) {
            return true;
          }
      ...
      typeFilter = filteredData.filter(
        (item) => item.measurementType === selectedType
      );
      gradeFilter = typeFilter;
      if (selectedGrade !== "All") {
        gradeFilter = typeFilter.filter(
          ...
          <Paper elevation={3}>
            <Box p={3}>
              ...
              <Typography variant="body1" paragraph>
                It is Pie Chart that provides a focused distinction on the BMI
                Classification among feeding program eligible student/s,
                allowing you to filter data by school year, month, grade and
                feeding measurement type.
              </Typography>
